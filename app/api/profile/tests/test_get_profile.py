...
from ...helpers.serialization_errors import error_dict, jwt_errors
...
class GetUserProfileTest(ProfileTestBaseCase):
    ...
    def test_get_profiles_with_expired_token_fails(self):
        """Test get profile with expired toke fails"""
        response = self.client.get(self.list_update_profile_url,
        HTTP_AUTHORIZATION='bearer {}'.format(
            self.expired_token))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn(jwt_errors['token_expired'], str(response.data))
